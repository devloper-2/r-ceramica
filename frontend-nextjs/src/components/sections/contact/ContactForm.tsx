    ...
    <div className="bg-white/[0.03] backdrop-blur-3xl p-8 md:p-10 border border-white/10 rounded-sm shadow-2xl relative overflow-hidden">
      ...
      <form className="space-y-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <ContactInput id="name" name="name" label="Full Name" required />
          <ContactInput id="email" name="email" type="email" label="Email Address" required />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <ContactInput id="phone" name="phone" type="tel" label="Phone Number" />
          <ContactSelect label="Subject" placeholder="Select Project Type"
            options={[
              {
                label: "Residential",
                value: "residential",
        ...
        <ContactTextarea id="message" name="message" label="Message / Requirements"/>
        <div className="pt-6">
          <Button type="submit" variant="solid" size="md" className="w-full md:w-auto flex items-center justify-center gap-4 font-bold tracking-[0.4em]">
            Submit Inquiry
            <ArrowRight
              size={18}
